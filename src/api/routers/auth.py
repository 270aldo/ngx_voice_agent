...
async def login(form_data: OAuth2PasswordRequestForm = Depends()) -> Dict[str, Any]:
    ...
    try:
        ...
        # Generar tokens JWT con JTI para session management
        import secrets
        ...
        token_data = {
            ...
            "jti": jti  # Add JWT ID for revocation support
        }
...
async def logout(current_user = Depends(get_current_user)) -> Dict[str, Any]:
    ...
    """
    try:
        # Get JWT token info from current user
        # Note: current_user should have token_data with jti
        if hasattr(current_user, 'token_data') and current_user.token_data:
            jti = current_user.token_data.get('jti')
            expires_at = current_user.token_data.get('exp', 0)
