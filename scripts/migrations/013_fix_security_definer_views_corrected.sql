-- =====================================================================
-- MIGRACIÓN 013 CORREGIDA: CORREGIR VISTAS SECURITY DEFINER
-- =====================================================================
-- Este script corrige:
-- 1. Errores de SECURITY DEFINER en vistas (48 errores)
-- 2. Problema de columna prompt_variant_id que no existe
-- Fecha: 2025-07-28

-- =====================================================================
-- PARTE 1: VERIFICAR Y CREAR COLUMNAS NECESARIAS
-- =====================================================================

-- Primero, verificar si conversation_outcomes existe y tiene las columnas necesarias
DO $$
BEGIN
    -- Si la tabla conversation_outcomes existe pero no tiene prompt_variant_id
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'conversation_outcomes') THEN
        -- Agregar columna prompt_variant_id si no existe
        IF NOT EXISTS (
            ...
            RAISE NOTICE 'Columna prompt_variant_id agregada a conversation_outcomes';
        END IF;
        
        -- Agregar columna converted si no existe
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'conversation_outcomes' 
            ...
            -- Actualizar basándose en el outcome existente
            UPDATE conversation_outcomes 
            SET converted = (outcome = 'converted')
            WHERE converted IS NULL;
            
            RAISE NOTICE 'Columna converted agregada a conversation_outcomes';
        END IF;
        
        -- Agregar columna quality_score si no existe
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'conversation_outcomes' 
            ...
            ALTER TABLE conversation_outcomes 
            ADD COLUMN quality_score FLOAT DEFAULT 0.0 CHECK (quality_score >= 0.0 AND quality_score <= 10.0);
            
            -- Calcular quality_score basándose en métricas existentes
            UPDATE conversation_outcomes 
            ...
            RAISE NOTICE 'Columna quality_score agregada a conversation_outcomes';
        END IF;
    END IF;
    
    -- Verificar y corregir el tipo de datos de variant_id en prompt_variants
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
    ...
    -- Asegurar que ab_test_variants tenga variant_id VARCHAR
...
-- 7. trial_performance_by_tier - Simplificada ya que no tenemos service_tiers
DROP VIEW IF EXISTS trial_performance_by_tier CASCADE;
CREATE VIEW trial_performance_by_tier
WITH (security_invoker = true)
AS
SELECT 
    c.tier_detected as tier_name,
    CASE 
...
WHERE c.tier_detected IS NOT NULL
...
-- 10. roi_by_profession_view - Simplificada ya que no tenemos lead_profiles
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'prompt_variants') THEN
        missing_tables := array_append(missing_tables, 'prompt_variants');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'conversation_outcomes') THEN
        missing_tables := array_append(missing_tables, 'conversation_outcomes');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'predictive_models') THEN
        missing_tables := array_append(missing_tables, 'predictive_models');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'training_jobs') THEN
        missing_tables := array_append(missing_tables, 'training_jobs');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'emotional_analyses') THEN
        missing_tables := array_append(missing_tables, 'emotional_analyses');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pattern_recognitions') THEN
        missing_tables := array_append(missing_tables, 'pattern_recognitions');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'genetic_evolutions') THEN
        missing_tables := array_append(missing_tables, 'genetic_evolutions');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'free_trials') THEN
        missing_tables := array_append(missing_tables, 'free_trials');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'demos') THEN
        missing_tables := array_append(missing_tables, 'demos');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'engagement_touchpoints') THEN
        missing_tables := array_append(missing_tables, 'engagement_touchpoints');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'roi_calculations') THEN
        missing_tables := array_append(missing_tables, 'roi_calculations');
    END IF;
    
    -- Reportar tablas faltantes
    IF array_length(missing_tables, 1) > 0 THEN
        FOREACH missing_table IN ARRAY missing_tables
        LOOP
            RAISE WARNING 'Tabla faltante: %, vista relacionada puede no funcionar correctamente', missing_table;
        END LOOP;
    END IF;
    ...
    RAISE NOTICE '✅ Migración 013 CORREGIDA completada';
    ...
    RAISE NOTICE '📋 Tablas faltantes encontradas: %', array_length(missing_tables, 1);
END $$;

-- =====================================================================
-- FIN DE MIGRACIÓN 013 CORREGIDA
-- =====================================================================